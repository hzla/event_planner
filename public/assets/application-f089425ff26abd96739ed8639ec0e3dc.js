...
(function() {
  ...
  Dashboard = {
    init: function() {
      $('body').on('click', '#create-poll', this.showPollForm);
      $('body').on('click touchend', '#cancel-form', this.hidePollForm);
      return $('body').on('ajax:success', '#activation-form', this.showActivation);
    },
...
(function() {
  ...
  DinnerPoll = {
    ...
    cycleImages: function() {
      var i, _i;
      for (i = _i = 0; _i <= 99999; i = ++_i) {
        window.clearInterval(i);
      }
      return setInterval(function() {
        var curText, current, next, nextText;
        ...
        if (next.hasClass('recycle')) {
          ...
          return $('#sec-3 .first').addClass('active');
        }
      }, 2000);
    }
  };
...
(function() {
  ...
  NewEvent = {
    ...
    finishTut: function() {
      return $('.tut-1, .tut-2').hide();
    },
    showNextArrow: function() {
      return $('.tut-1 #tut-right').show();
    },
  ...
  Options = {
    ...
    toggleOptionSelect: function() {
      ...
      if ($(this).hasClass('selected')) {
        ...
        return Options.addOptionToForm($(this));
      } else {
        id = $(this).attr('id');
        $('.selected-options').find("." + id).remove();
        return Options.removeOptionFromForm($(this));
      }
    },
...
(function() {
  ...
  Results = {
    init: function() {
      return $('body').on('click', '#see-more-results', this.seeMoreResults);
    },
...
(function() {
  ...
  SimpleNewEvent = {
    init: function() {
      $('.type .date-type').click(this.toggleEventDateForm);
      $('body').on('click', '.question-info-container .date-type', this.editDateQuestion);
      $('body').on('click', '.question-info-container .text-type', this.editTextQuestion);
      $('.text-type').click(this.toggleEventTextForm);
      $('body').on('click', '#cancel-simple-event', this.clearEventDetails);
      $('body').on('click', '#cancel-type-container', this.cancelTypeContainer);
      $('body').on('click', '#confirm-simple-event', this.confirmEventDetails);
      ...
      return $('.submit-simple-event').click(this.submitSimpleEvent);
    },
    cancelTypeContainer: function() {
      if (SimpleNewEvent.questionCount > 1) {
        $('.question-info-container').last().remove();
        $('.question-info-container').last().addClass('active');
        $('.type-container').hide();
        return $('#add-another-question').show();
      }
    },
    editDateQuestion: function() {
      var clickedQuestion, datesToSet, parsedDates;
      if ($('#text-choice-picker:visible, #datepicker:visible, .type-container:visible').length < 1) {
        SimpleNewEvent.editMode = true;
        $('.active').removeClass('active');
        clickedQuestion = $(this).parents(".question-info-container");
        ...
        return $('#datepicker, #simple-event-btns').show();
      }
    },
    editTextQuestion: function() {
      var clickedQuestion, textToSet;
      if ($('#text-choice-picker:visible, #datepicker:visible, .type-container:visible').length < 1) {
        SimpleNewEvent.editMode = true;
        $('.active').removeClass('active');
        clickedQuestion = $(this).parents(".question-info-container");
        ...
        return $.each(textToSet, function(i, choice) {
          var input;
          if (choice !== "") {
    ...
    cancelChoice: function() {
      ...
      return SimpleNewEvent.reassignNumbers();
    },
    addChoice: function() {
      ...
      nextChoice = "<div class='text-choice placeholder'> <div class='text-choice-num'>" + nextChoiceNum + ".</div> <input class='text-choice-input' id='text_choice_" + nextChoiceNum + "' name='text_choice_" + nextChoiceNum + "' type='text' disabled='disabled'> </div>";
      choice.after(nextChoice);
      $('.text-choice').last().append(addIcon);
      return $('.text-choice').last().append(cancelIcon);
    },
    reassignNumbers: function() {
      var count;
      count = 1;
      return $('.text-choice').each(function() {
        $(this).find('.text-choice-num').text("" + count + ".");
        return count += 1;
      });
    },
    addAnotherQuestion: function() {
      ...
      nextQuestion = "<div class='question-info-container active'> <input class='poll-field' id='question_" + qc + "' name='question__" + qc + "' placeholder='Type question...' type='text'> <input class='date-choices' id='date_choice_list' name='date_choice_list_" + qc + "' type='hidden'> <input class='text-choices' id='text_choice_list' name='text_choice_list_" + qc + "' type='hidden'> </div>";
      $(this).before(nextQuestion);
      $('#add-another-question').hide();
      return $('.type-container').show();
    },
    confirmEventDetails: function() {
      var currentQuestion, dates, icon, showTypePicker;
    ...
    toggleEventDateForm: function() {
      ...
      if (currentQuestion.val() !== "") {
        return $('#datepicker, #simple-event-btns, .type-container').toggle();
      } else {
    ...
    toggleEventTextForm: function() {
      ...
      if (currentQuestion.val() !== "") {
        ...
        return $('#text-choice-picker').css('height', "calc(100vh - " + offset + "px)");
      } else {
        currentQuestion.css('border', '1px solid red');
        return setTimeout(function() {
          return currentQuestion.attr('style', 'none');
    ...
    resetTextPicker: function() {
      ...
      return $('.text-choice-input').last().attr('disabled', 'disabled');
    }
  };
...
(function() {
  ...
  SimplePollVoting = {
    init: function() {
      ...
      $('.datepicker, .text-choice').click(this.updateVotedStatus);
    ...
    initDateVoter: function() {
      ...
      $('.simple-choice').each(function() {
        ...
        return dateVoter.datepicker("setDates", parsedSelectedDates);
      });
      return $('.dow').parent().addClass('dow-row');
    },
    toggleVoter: function() {
      return $(this).parent().find('.simple-voter').toggle();
    },
