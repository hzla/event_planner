...
(function() {
  ...
  Poll = {
    ...
    choose: function(event, data) {
      var count, current_score, delta, gray, new_score;
      count = $(this).parent();
      delta = data.delta;
      if (data.changed) {
        if (data.answer === "yes") {
          ...
          gray = $(this).parents('.choice-vote-container').find('.downvote-link').find('g').first().css('stroke') === "rgb(255, 0, 67)";
          $(this).parent().find('g').css('stroke', '#bebebe');
          if (!gray) {
            $(this).parent().find('g').css('stroke', '#bebebe');
            $(this).find('g').css('stroke', '#00CC99');
            return current_score.css('color', '#00CC99');
          } else {
            $(this).parent().find('g').css('stroke', '#bebebe');
            $(this).find('g').css('stroke', '#bebebe');
            return current_score.css('color', '#bebebe');
          }
        } else {
          current_score = count.find('.choice-score');
          new_score = parseInt(current_score.text()) - delta;
          ...
          gray = $(this).parents('.choice-vote-container').find('.upvote-link').find('g').first().css('stroke') === "rgb(0, 204, 153)";
          $(this).parent().find('g').css('stroke', '#bebebe');
          if (!gray) {
            $(this).find('g').css('stroke', '#FF0043');
            return current_score.css('color', '#FF0043');
          } else {
            $(this).parent().find('g').css('stroke', '#bebebe');
            $(this).find('g').css('stroke', '#bebebe');
            return current_score.css('color', '#bebebe');
  ...
  SimplePollVoting = {
    ...
    submitEventVotes: function() {
      ...
      $('.date-voter').each(function() {
        var dates, dates_with_questions, question;
        dates = $(this).datepicker('getDates');
        dates_with_questions = [];
        question = $(this).parent().attr('data-question');
        $.each(dates, function(i, v) {
          return dates_with_questions.push(v + ("<question>" + question));
        });
      ...
      $('.text-choice.selected .text-choice-input').each(function() {
        var question;
        question = $(this).parents('.text-choice').attr('data-question');
        return values += "<separator>" + $(this).text() + ("<question>" + question);
      });
      console.log(values);
      values = values.replace(/,/g, "<separator>");
      $('#choice_values').val("");
    ...
    toggleVoter: function() {
      ...
      if ($('.day:visible').length > 0) {
        ...
        while ($('.day:not(.disabled):visible').length < 1) {
          ...
          _results.push(console.log("clicking"));
        }
        return _results;
      }
    },
...
(function() {
  ...
  SimpleNewEventKeydown = {
    init: function() {
      ...
      return $('body').on('keydown', '.text-choice-input', this.checkIfPollDone);
    },
    checkIfPollDone: function(e) {
      if ($('.text-choice-input').first().val() !== "" && $($('.text-choice-input')[1]).val() !== "" && $('#event_name').val() !== "") {
        return $('.submit-simple-event').removeClass('inactive');
      }
    },
    ...
    showTypePicker: function() {
      ...
      if (SimpleNewEvent.shown === false && $('.date-picker-container:visible, #text-choice-picker:visible').length < 1 && $(this).val() !== "") {
        ...
        $('.type-container').animate({
          ...
          return $('#poll-creator, #simple-events-form, body, #content-container').scrollTop(100000);
        });
...
(function() {
  ...
  SimpleNewEventUi = {
    ...
    addChoice: function() {
      ...
      nextChoice = "<div class='text-choice placeholder'> <div class='draggable-container'> <div class='draggable-top'></div> <div class='draggable-bottom'></div> <div class='text-choice-num'>" + (String.fromCharCode(nextChoiceNum)) + ".</div> </div> <input class='text-choice-input' id='text_choice_" + (nextChoiceNum - 96) + "' name='text_choice_" + nextChoiceNum + "' type='text' placeholder='Add Option...'> </div>";
...
(function() {
  ...
  SimpleEventResults = {
    ...
    expandResultsTable: function() {
      ...
      return resultsTable.find('.simple-results-header, .simple-choice-body').css('width', "" + width + "px");
    },
